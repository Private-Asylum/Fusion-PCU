//! Compare wrapping u16 PCU Add with independent native HIP.
...
#[pcu(invocations = N)]
fn wrapping_add_u16<const N: usize>(left: &[u16], right: &[u16], output: &mut [u16]) {
    let id = pcu::context::global_invocation_id();
    output[id] = left[id].wrapping_add(right[id]);
}

#[pcu(invocations = 64)]
fn wrapping_add_grid_u16<const N: usize>(left: &[u16], right: &[u16], output: &mut [u16]) {
...
fn bench(criterion: &mut Criterion) {
    run(criterion).expect("u16 wrapping Add benchmark failed");
}
...
fn run(criterion: &mut Criterion) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "u16 wrapping Add device: {} ({architecture})",
...
fn run_case<const N: usize>(
    ...
    let left = (0..N)
        .map(|i| {
            if i == 0 {
                ...
                u16::try_from(i % (usize::from(u16::MAX) + 1))
                    .expect("masked u16 input fits")
                    .wrapping_mul(251)
            }
        })
        .collect::<Vec<_>>();
    ...
    let bindings = wrapping_add_u16_bindings();
    let builder = wrapping_add_u16::<N>(&bindings)?;
    let kernel = builder.ir();
    let invocations = u32::try_from(N)?;
    let prepared = support::cold_once("PCU u16 cold prepare", || {
    ...
    let grid_bindings = wrapping_add_grid_u16_bindings();
    let grid_builder = wrapping_add_grid_u16::<N>(&grid_bindings)?;
    let grid_kernel = grid_builder.ir();
    let grid_prepared = support::cold_once("PCU u16 grid-stride cold prepare", || {
        backend.prepare_dispatch(PcuDispatchSubmission {
            kernel: &grid_kernel,
    ...
    let source = String::from(
        "#include <hip/hip_runtime.h>\nextern \"C\" __global__ void native_u16_direct(const unsigned short* left, const unsigned short* right, unsigned short* output, unsigned int n) { unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; if (id < n) output[id] = static_cast<unsigned short>(left[id] + right[id]); }\nextern \"C\" __global__ void native_u16_grid(const unsigned short* left, const unsigned short* right, unsigned short* output, unsigned int n) { unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; unsigned int stride = gridDim.x * blockDim.x; for (; id < n; id += stride) output[id] = static_cast<unsigned short>(left[id] + right[id]); }\n",
    );
    let image = compile_hip_source(&source, architecture)?;
    let module = runtime.load_module(&image)?;
    let direct = module.function(c"native_u16_direct")?;
    let grid_function = module.function(c"native_u16_grid")?;
    ...
    println!("u16 wrapping Add direct and grid-stride verified for {N} values, including overflow");

    let mut group = criterion.benchmark_group("u16-wrapping-add");
            ...
            bencher.iter_custom(|iterations| {
                ...
                for iteration in 0..iterations {
                    let (pcu, native) = if iteration.is_multiple_of(2) {
                        ...
                        .expect("PCU u16 launch failed");
                        let native = run_direct(&direct, &stream, &direct_args, grid)
                            .expect("native u16 launch failed");
                        (pcu, native)
                    } else {
                        let native = run_direct(&direct, &stream, &direct_args, grid)
                            .expect("native u16 launch failed");
...
fn verify(
    ...
    if pcu_bytes != expected_bytes || native_bytes != expected_bytes {
        return Err(format!("u16 {phase} PCU/native output mismatch").into());
    }
    Ok(())
}
