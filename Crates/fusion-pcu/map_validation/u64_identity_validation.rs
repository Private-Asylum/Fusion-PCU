...
/// Structural failure within the scalar `u64` invocation-index identity profile.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PcuU64IdentityValidationError {
...
/// Validates one `u64` storage copy per invocation, directly or through a grid-stride loop.
///
...
pub fn validate_u64_identity_kernel(
    kernel: &PcuDispatchKernelIr<'_>,
) -> Result<(), PcuU64IdentityValidationError> {
    if !kernel.ports.is_empty() || !kernel.parameters.is_empty() || kernel.bindings.len() != 2 {
        return Err(PcuU64IdentityValidationError::UnsupportedInterface);
    }
    let types = PcuValueTypeCaps::UINT64 | PcuValueTypeCaps::SCALAR_VALUES;
    let features =
    ...
    {
        return Err(PcuU64IdentityValidationError::UnsupportedRequirements);
    }
    for (index, binding) in kernel.bindings.iter().enumerate() {
        ...
        {
            return Err(PcuU64IdentityValidationError::DuplicateBinding(reference));
        }
        if binding.storage != PcuBindingStorageClass::Storage
            || binding.builtin.is_some()
            || binding.binding_type != PcuBindingType::Value(PcuValueType::u64())
        {
            return Err(PcuU64IdentityValidationError::InvalidBinding(reference));
        }
    }
    ...
    {
        if *extent == 0 {
            return Err(PcuU64IdentityValidationError::UnsupportedOperation(0));
        }
    ...
    if kernel.ops.len() < 3 {
        return Err(match kernel.ops.len() {
            0 => PcuU64IdentityValidationError::MissingLoad,
            1 => PcuU64IdentityValidationError::MissingStore,
            _ => PcuU64IdentityValidationError::MissingReturn,
        });
    }
    if kernel.ops.len() > 3 {
        return Err(PcuU64IdentityValidationError::UnsupportedOperation(3));
    }
    let (loaded_value, input) = match kernel.ops[0] {
        ...
        PcuDispatchOp::Data(PcuDispatchDataOp::BindingLoad { .. }) => {
            return Err(PcuU64IdentityValidationError::InvalidIndex(0));
        }
        _ => return Err(PcuU64IdentityValidationError::MissingLoad),
    };
    if loaded_value.0 == 0 {
        return Err(PcuU64IdentityValidationError::InvalidValue(loaded_value));
    }
    ...
    match kernel.ops[1] {
        PcuDispatchOp::Data(PcuDispatchDataOp::BindingStore {
            ...
            if value != loaded_value {
                return Err(PcuU64IdentityValidationError::InvalidValue(value));
            }
            if binding == input {
                return Err(PcuU64IdentityValidationError::InvalidBinding(binding));
            }
            validate_binding(kernel, binding, true)?;
        }
        PcuDispatchOp::Data(PcuDispatchDataOp::BindingStore { .. }) => {
            return Err(PcuU64IdentityValidationError::InvalidIndex(1));
        }
        _ => return Err(PcuU64IdentityValidationError::MissingStore),
    }
    if !matches!(
        kernel.ops[2],
        PcuDispatchOp::Control(PcuDispatchControlOp::Return)
    ) {
        return Err(PcuU64IdentityValidationError::MissingReturn);
    }
    Ok(())
}
...
fn validate_grid_stride_copy(
    ...
    if input == output {
        return Err(PcuU64IdentityValidationError::InvalidBinding(*output));
    }
    validate_binding(kernel, *input, false)?;
...
fn validate_binding(
    ...
    let Some(binding) = kernel
        ...
        return Err(PcuU64IdentityValidationError::InvalidBinding(target));
    };
    if (write && binding.access == PcuBindingAccess::ReadOnly)
        || (!write && binding.access == PcuBindingAccess::WriteOnly)
    {
        Err(PcuU64IdentityValidationError::InvalidBinding(target))
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn accepts_only_typed_u64_invocation_identity_copy() {
        ...
        assert_eq!(validate_u64_identity_kernel(&kernel), Ok(()));
        ...
        assert_eq!(
            validate_u64_identity_kernel(&PcuDispatchKernelIr {
                ops: &grid_ops,
                ..kernel
            }),
            Ok(())
