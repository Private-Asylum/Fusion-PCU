//! Structural admission for the bounded scalar `i16` arithmetic map profile.
...
/// Structural failure within the typed scalar `i16` arithmetic map profile.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PcuI16MapValidationError {
...
const fn map_error(error: IntegerMapValidationError) -> PcuI16MapValidationError {
    match error {
        ...
        IntegerMapValidationError::InvalidBinding(value) => {
            PcuI16MapValidationError::InvalidBinding(value)
        }
        IntegerMapValidationError::DuplicateBinding(value) => {
            PcuI16MapValidationError::DuplicateBinding(value)
        }
        IntegerMapValidationError::UnsupportedOperation(value) => {
            PcuI16MapValidationError::UnsupportedOperation(value)
        }
        IntegerMapValidationError::InvalidIndex(value) => {
            PcuI16MapValidationError::InvalidIndex(value)
        }
        IntegerMapValidationError::InvalidValue(value) => {
            PcuI16MapValidationError::InvalidValue(value)
        }
        IntegerMapValidationError::MissingReturn => PcuI16MapValidationError::MissingReturn,
    }
}

/// Validates indexed `i16` loads, one or more wrapping Add/Sub/Mul operations, one store and Return.
///
...
/// by the dispatch ABI. `i16` division and remainder are outside this profile.
///
/// # Errors
///
/// Returns the first structural violation of the typed `i16` map profile.
pub fn validate_i16_map_kernel(
    kernel: &PcuDispatchKernelIr<'_>,
) -> Result<(), PcuI16MapValidationError> {
    validate_integer_map_kernel(kernel, PcuValueType::i16(), PcuValueTypeCaps::INT16)
        .map_err(map_error)
}
...
mod tests {
    ...
    fn kernel(extent: Option<u32>, alu: PcuDispatchAluOp) -> PcuDispatchKernelIr<'static> {
        ...
        let body = Box::leak(Box::new([
            ...
            PcuDispatchOp::Data(PcuDispatchDataOp::Alu {
                value_type: crate::PcuValueType::i16(),
    ...
    fn admits_wrapping_arithmetic_and_large_grid_extent_but_not_division() {
        for op in [
            ...
            assert_eq!(validate_i16_map_kernel(&kernel(None, op)), Ok(()));
            assert_eq!(validate_i16_map_kernel(&kernel(Some(u32::MAX), op)), Ok(()));
        }
        assert!(matches!(
            validate_i16_map_kernel(&kernel(None, PcuDispatchAluOp::Div)),
            Err(PcuI16MapValidationError::UnsupportedOperation(2))
        ));
    }
    ...
    fn admits_chained_wrapping_alu_direct_and_grid_stride() {
        assert_eq!(validate_i16_map_kernel(&chained_kernel(None)), Ok(()));
        assert_eq!(validate_i16_map_kernel(&chained_kernel(Some(100))), Ok(()));
    }
}
