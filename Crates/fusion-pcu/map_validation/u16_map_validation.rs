...
/// Structural failure within the typed scalar `u16` arithmetic map profile.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PcuU16MapValidationError {
...
const fn map_error(error: IntegerMapValidationError) -> PcuU16MapValidationError {
    match error {
        ...
        IntegerMapValidationError::InvalidBinding(value) => {
            PcuU16MapValidationError::InvalidBinding(value)
        }
        IntegerMapValidationError::DuplicateBinding(value) => {
            PcuU16MapValidationError::DuplicateBinding(value)
        }
        IntegerMapValidationError::UnsupportedOperation(value) => {
            PcuU16MapValidationError::UnsupportedOperation(value)
        }
        IntegerMapValidationError::InvalidIndex(value) => {
            PcuU16MapValidationError::InvalidIndex(value)
        }
        IntegerMapValidationError::InvalidValue(value) => {
            PcuU16MapValidationError::InvalidValue(value)
        }
        IntegerMapValidationError::MissingReturn => PcuU16MapValidationError::MissingReturn,
    }
}

/// Validates indexed `u16` loads, one or more wrapping Add/Sub/Mul operations, one store and Return.
///
...
/// by the dispatch ABI. `u16` division and remainder are outside this profile.
///
/// # Errors
///
/// Returns the first structural violation of the typed `u16` map profile.
pub fn validate_u16_map_kernel(
    kernel: &PcuDispatchKernelIr<'_>,
) -> Result<(), PcuU16MapValidationError> {
    validate_integer_map_kernel(kernel, PcuValueType::u16(), PcuValueTypeCaps::UINT16)
        .map_err(map_error)
}
...
mod tests {
    ...
    fn kernel(extent: Option<u32>, alu: PcuDispatchAluOp) -> PcuDispatchKernelIr<'static> {
        ...
        let body = Box::leak(Box::new([
            ...
            PcuDispatchOp::Data(PcuDispatchDataOp::Alu {
                value_type: crate::PcuValueType::u16(),
    ...
    fn admits_wrapping_arithmetic_and_large_grid_extent_but_not_division() {
        for op in [
            ...
            assert_eq!(validate_u16_map_kernel(&kernel(None, op)), Ok(()));
            assert_eq!(validate_u16_map_kernel(&kernel(Some(u32::MAX), op)), Ok(()));
        }
        assert!(matches!(
            validate_u16_map_kernel(&kernel(None, PcuDispatchAluOp::Div)),
            Err(PcuU16MapValidationError::UnsupportedOperation(2))
        ));
    }
    ...
    fn admits_chained_wrapping_alu_direct_and_grid_stride() {
        assert_eq!(validate_u16_map_kernel(&chained_kernel(None)), Ok(()));
        assert_eq!(validate_u16_map_kernel(&chained_kernel(Some(100))), Ok(()));
    }
}
