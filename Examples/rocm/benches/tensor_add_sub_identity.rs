//! Resident comparison of separate PCU plain Add/Sub chain, grouped PCU plain Add/Sub chain, and one native HIP kernel.
...
#[path = "support/add_sub_chain_identity.rs"]
mod native;
#[path = "support/add_relu_pair.rs"]
mod paired;

fn bench(criterion: &mut Criterion) {
    run(criterion).expect("ROCm plain Add/Sub chain fusion benchmark failed");
}
...
fn run(criterion: &mut Criterion) -> Result<(), Box<dyn Error>> {
    ...
    Err(format!(
        "no ROCm device completed plain Add/Sub chain fusion benchmark: {}",
        failures.join("; ")
    )
    .into())
}
...
fn run_on(
    ...
    println!(
        "Device: {}; resident separate PCU plain Add/Sub chain vs grouped PCU vs native HIP single kernel",
        selected.name
    );
    ...
    for elements in [65_usize, 1_048_576] {
        ...
        let separate = support::cold_once(
            &format!("PCU separate plain Add/Sub chain {elements} cold graph preparation"),
            || {
        ...
        let grouped = support::cold_once(
            &format!("PCU grouped plain Add/Sub chain {elements} cold graph preparation"),
            || {
                assessor.prepare_graph_outputs_with_policies(
                    &graph,
                    &[output],
        ...
        let native = support::cold_once(
            ...
            || {
                native::NativeAddSubChainIdentity::prepare(
                    discovery,
                    selected.device,
                    &left_values,
                    &right_values,
        ...
        let mut group = criterion.benchmark_group(format!("tensor_add_sub_identity/{elements}"));
        ...
        group.bench_function("pcu_prepared_separate_resident", |bencher| {
            bencher.iter(|| {
                assessor
                    ...
                    .expect("separate PCU plain Add/Sub chain execution failed");
            });
        });
        group.bench_function("pcu_prepared_grouped_resident", |bencher| {
            bencher.iter(|| {
                assessor
                    ...
                    .expect("grouped PCU plain Add/Sub chain execution failed");
            });
        });
        group.bench_function("native_hip_single_kernel_resident", |bencher| {
            bencher.iter(|| {
                native
                    .execute_resident()
                    .expect("native HIP plain Add/Sub chain failed");
            });
        });
        group.finish();
...
fn verify(expected: &[f32], actual: &[f32]) -> Result<(), Box<dyn Error>> {
            ...
            .all(|(expected, actual)| expected.to_bits() == actual.to_bits())
    {
        ...
        Err("plain Add/Sub chain result mismatch".into())
    }
}
