//! Compare wrapping i8 PCU Add with independent native HIP.
...
#[pcu(invocations = N)]
fn wrapping_add_i8<const N: usize>(left: &[i8], right: &[i8], output: &mut [i8]) {
    let id = pcu::context::global_invocation_id();
    output[id] = left[id].wrapping_add(right[id]);
}

#[pcu(invocations = 64)]
fn wrapping_add_grid_i8<const N: usize>(left: &[i8], right: &[i8], output: &mut [i8]) {
...
fn bench(criterion: &mut Criterion) {
    run(criterion).expect("i8 wrapping Add benchmark failed");
}
...
fn run(criterion: &mut Criterion) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "i8 wrapping Add device: {} ({architecture})",
...
fn run_case<const N: usize>(
    ...
    let left = (0..N)
        .map(|i| match i {
            0 => i8::MAX,
            1 => i8::MIN,
            _ => i8::try_from(i % (usize::from(u8::MAX) / 2 + 1))
                .expect("bounded i8 input fits")
                .wrapping_mul(-5),
        })
        .collect::<Vec<_>>();
    let right = (0..N)
    ...
    let source = String::from(
        "#include <hip/hip_runtime.h>\nextern \"C\" __global__ void native_i8_direct(const unsigned char* left, const unsigned char* right, unsigned char* output, unsigned int n) { unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; if (id < n) output[id] = static_cast<unsigned char>((static_cast<unsigned int>(left[id]) + static_cast<unsigned int>(right[id])) & 0xffu); }\nextern \"C\" __global__ void native_i8_grid(const unsigned char* left, const unsigned char* right, unsigned char* output, unsigned int n) { unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; unsigned int stride = gridDim.x * blockDim.x; for (; id < n; id += stride) output[id] = static_cast<unsigned char>((static_cast<unsigned int>(left[id]) + static_cast<unsigned int>(right[id])) & 0xffu); }\n",
    );
    let image = compile_hip_source(&source, architecture)?;
    let module = runtime.load_module(&image)?;
    let direct = module.function(c"native_i8_direct")?;
    let grid_function = module.function(c"native_i8_grid")?;
    ...
    println!("i8 wrapping Add direct and grid-stride verified for {N} values, including overflow");

    let mut group = criterion.benchmark_group("i8-wrapping-add");
            ...
            bencher.iter_custom(|iterations| {
                ...
                for iteration in 0..iterations {
                    let (pcu, native) = if iteration.is_multiple_of(2) {
                        ...
                        .expect("PCU i8 launch failed");
                        let native = run_direct(&direct, &stream, &direct_args, grid)
                            .expect("native i8 launch failed");
                        (pcu, native)
                    } else {
                        let native = run_direct(&direct, &stream, &direct_args, grid)
                            .expect("native i8 launch failed");
...
fn verify(
    ...
    if pcu_bytes != expected_bytes || native_bytes != expected_bytes {
        return Err(format!("i8 {phase} PCU/native output mismatch").into());
    }
    Ok(())
}
