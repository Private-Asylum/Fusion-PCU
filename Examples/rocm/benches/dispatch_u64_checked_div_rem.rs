//! Checked u64 quotient/remainder correctness and native HIP comparison.
...
fn bench(criterion: &mut Criterion) {
    run(criterion).expect("checked u64 DivRem benchmark setup failed");
}
...
fn run(criterion: &mut Criterion) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "checked u64 DivRem device: {} ({architecture}); PCU payload buffers are reused; each timed PCU sample includes backend fault-word allocation/init, launch, wait, and status readback; native includes the matching HIP allocation/init/launch/wait/readback",
...
fn run_case<const N: usize>(
    ...
    let left = (0..N)
        .map(|i| (1_u64 << 48) + u64::try_from(i).expect("index fits") * 0x1_0000_0123 + 5)
        .collect::<Vec<_>>();
    let right = (0..N)
        .map(|i| (1_u64 << 32) + u64::try_from(i % 31 + 1).expect("divisor fits"))
        .collect::<Vec<_>>();
    let expected_q = left
        .iter()
        .zip(&right)
    ...
    let expected_r = left
        ...
        .map(|(a, b)| a % b)
        .collect::<Vec<_>>();
    let lhs_bytes = encode_u64(&left);
    let rhs_bytes = encode_u64(&right);
    ...
    let kernel = PcuDispatchKernelIr {
        ...
        parameters: &[],
        ops: kernel_ops,
        type_caps: PcuValueTypeCaps::UINT64 | PcuValueTypeCaps::SCALAR_VALUES,
    ...
    let function = module.function(c"native_checked_u64_div_rem")?;
    ...
    println!(
        "checked u64 DivRem verified extent={N}, invocations={invocations}, grid_stride={grid_stride}"
    );
    ...
    if !grid_stride {
        ...
        let mut group = criterion.benchmark_group("u64-checked-div-rem-direct");
...
fn verify_pair(
    ...
    let mut q = vec![0; expected_q.len() * 8];
    let mut r = vec![0; expected_r.len() * 8];
    quotient.copy_to(&mut q)?;
    remainder.copy_to(&mut r)?;
    let decode = |bytes: &[u8]| {
        bytes
            .chunks_exact(8)
            .map(|x| u64::from_le_bytes(x.try_into().expect("eight bytes")))
            .collect::<Vec<_>>()
    };
...
fn verify_fault(
    ...
    let lhs = runtime.allocate(n * 8)?;
    let mut rhs = runtime.allocate(n * 8)?;
    let quotient = runtime.allocate(n * 8)?;
    let remainder = runtime.allocate(n * 8)?;
    rhs.copy_from(&encode_u64(&divisors))?;
    let mut status = runtime.allocate(8)?;
    status.copy_from(&u64::MAX.to_le_bytes())?;
    let mut word = [0_u8; 8];
    {
...
fn native_source(extent: u32, invocations: u32, grid_stride: bool) -> String {
    ...
    let close = "}";
    format!(
        "#include <hip/hip_runtime.h>\nextern \"C\" __global__ void native_checked_u64_div_rem(const unsigned long long* a,const unsigned long long* b,unsigned long long* q,unsigned long long* r,unsigned long long* fault_word) {{\nunsigned int base=blockIdx.x*blockDim.x+threadIdx.x; if (base >= {invocations}u) return; {iteration} if (b[i] == 0ull) {{ atomicMin(fault_word, (static_cast<unsigned long long>(i) << 2u) | 1ull); }} else {{ q[i]=a[i]/b[i]; r[i]=a[i]%b[i]; }} {close} }}\n"
    )
}
