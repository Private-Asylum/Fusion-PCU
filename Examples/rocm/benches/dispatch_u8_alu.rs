//! Compare wrapping u8 PCU Add with independent native HIP.
...
#[pcu(invocations = N)]
fn wrapping_add_u8<const N: usize>(left: &[u8], right: &[u8], output: &mut [u8]) {
    let id = pcu::context::global_invocation_id();
    output[id] = left[id].wrapping_add(right[id]);
}

#[pcu(invocations = 64)]
fn wrapping_add_grid_u8<const N: usize>(left: &[u8], right: &[u8], output: &mut [u8]) {
...
fn bench(criterion: &mut Criterion) {
    run(criterion).expect("u8 wrapping Add benchmark failed");
}
...
fn run(criterion: &mut Criterion) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "u8 wrapping Add device: {} ({architecture})",
...
fn run_case<const N: usize>(
    ...
    let left = (0..N)
        .map(|i| {
            if i == 0 {
                ...
                u8::try_from(i % (usize::from(u8::MAX) + 1))
                    .expect("masked u8 input fits")
                    .wrapping_mul(251)
            }
        })
        .collect::<Vec<_>>();
    ...
    let bindings = wrapping_add_u8_bindings();
    let builder = wrapping_add_u8::<N>(&bindings)?;
    let kernel = builder.ir();
    let invocations = u32::try_from(N)?;
    let prepared = support::cold_once("PCU u8 cold prepare", || {
    ...
    let grid_bindings = wrapping_add_grid_u8_bindings();
    let grid_builder = wrapping_add_grid_u8::<N>(&grid_bindings)?;
    let grid_kernel = grid_builder.ir();
    let grid_prepared = support::cold_once("PCU u8 grid-stride cold prepare", || {
        backend.prepare_dispatch(PcuDispatchSubmission {
            kernel: &grid_kernel,
    ...
    let source = String::from(
        "#include <hip/hip_runtime.h>\nextern \"C\" __global__ void native_u8_direct(const unsigned char* left, const unsigned char* right, unsigned char* output, unsigned int n) { unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; if (id < n) output[id] = static_cast<unsigned char>((static_cast<unsigned int>(left[id]) + static_cast<unsigned int>(right[id])) & 0xffu); }\nextern \"C\" __global__ void native_u8_grid(const unsigned char* left, const unsigned char* right, unsigned char* output, unsigned int n) { unsigned int id = blockIdx.x * blockDim.x + threadIdx.x; unsigned int stride = gridDim.x * blockDim.x; for (; id < n; id += stride) output[id] = static_cast<unsigned char>((static_cast<unsigned int>(left[id]) + static_cast<unsigned int>(right[id])) & 0xffu); }\n",
    );
    let image = compile_hip_source(&source, architecture)?;
    let module = runtime.load_module(&image)?;
    let direct = module.function(c"native_u8_direct")?;
    let grid_function = module.function(c"native_u8_grid")?;
    ...
    println!("u8 wrapping Add direct and grid-stride verified for {N} values, including overflow");

    let mut group = criterion.benchmark_group("u8-wrapping-add");
            ...
            bencher.iter_custom(|iterations| {
                ...
                for iteration in 0..iterations {
                    let (pcu, native) = if iteration.is_multiple_of(2) {
                        ...
                        .expect("PCU u8 launch failed");
                        let native = run_direct(&direct, &stream, &direct_args, grid)
                            .expect("native u8 launch failed");
                        (pcu, native)
                    } else {
                        let native = run_direct(&direct, &stream, &direct_args, grid)
                            .expect("native u8 launch failed");
...
fn verify(
    ...
    let mut pcu_bytes = vec![0; n];
    let mut native_bytes = vec![0; n];
    ...
    if pcu_bytes != expected_bytes || native_bytes != expected_bytes {
        return Err(format!("u8 {phase} PCU/native output mismatch").into());
    }
    Ok(())
}
