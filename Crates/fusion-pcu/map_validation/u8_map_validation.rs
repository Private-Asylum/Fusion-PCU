//! Structural admission for the bounded scalar `u8` arithmetic map profile.
...
/// Structural failure within the typed scalar `u8` arithmetic map profile.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PcuU8MapValidationError {
...
const fn map_error(error: IntegerMapValidationError) -> PcuU8MapValidationError {
    match error {
        ...
        IntegerMapValidationError::InvalidBinding(value) => {
            PcuU8MapValidationError::InvalidBinding(value)
        }
        IntegerMapValidationError::DuplicateBinding(value) => {
            PcuU8MapValidationError::DuplicateBinding(value)
        }
        IntegerMapValidationError::UnsupportedOperation(value) => {
            PcuU8MapValidationError::UnsupportedOperation(value)
        }
        IntegerMapValidationError::InvalidIndex(value) => {
            PcuU8MapValidationError::InvalidIndex(value)
        }
        IntegerMapValidationError::InvalidValue(value) => {
            PcuU8MapValidationError::InvalidValue(value)
        }
        IntegerMapValidationError::MissingReturn => PcuU8MapValidationError::MissingReturn,
    }
}

/// Validates indexed `u8` loads, one or more wrapping Add/Sub/Mul operations, one store and Return.
///
...
/// by the dispatch ABI. `u8` division and remainder are outside this profile.
///
/// # Errors
///
/// Returns the first structural violation of the typed `u8` map profile.
pub fn validate_u8_map_kernel(
    kernel: &PcuDispatchKernelIr<'_>,
) -> Result<(), PcuU8MapValidationError> {
    validate_integer_map_kernel(kernel, PcuValueType::u8(), PcuValueTypeCaps::UINT8)
        .map_err(map_error)
}
...
mod tests {
    ...
    fn kernel(extent: Option<u32>, alu: PcuDispatchAluOp) -> PcuDispatchKernelIr<'static> {
        ...
        let body = Box::leak(Box::new([
            ...
            PcuDispatchOp::Data(PcuDispatchDataOp::Alu {
                value_type: crate::PcuValueType::u8(),
    ...
    fn admits_wrapping_arithmetic_and_large_grid_extent_but_not_division() {
        for op in [
            ...
            assert_eq!(validate_u8_map_kernel(&kernel(None, op)), Ok(()));
            assert_eq!(validate_u8_map_kernel(&kernel(Some(u32::MAX), op)), Ok(()));
        }
        assert!(matches!(
            validate_u8_map_kernel(&kernel(None, PcuDispatchAluOp::Div)),
            Err(PcuU8MapValidationError::UnsupportedOperation(2))
        ));
    }
    ...
    fn admits_chained_wrapping_alu_direct_and_grid_stride() {
        assert_eq!(validate_u8_map_kernel(&chained_kernel(None)), Ok(()));
        assert_eq!(validate_u8_map_kernel(&chained_kernel(Some(100))), Ok(()));
    }
}
