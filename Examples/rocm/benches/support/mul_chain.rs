//! Resident native HIP peer for the Mul-only chain fusion benchmark.
...
const SOURCE: &str = r#"
extern "C" __global__ void native_mul_chain(
...
/// Native one-kernel Mul/Mul/Mul with the same resident input/output boundary as PCU.
pub struct NativeMulChain {
    _runtime: HipRuntime,
...
impl NativeMulChain {
    pub fn prepare(
        ...
        if left.is_empty() || left.len() != right.len() || left.len() != third.len() {
            return Err("native Mul chain input shapes must be equal and nonempty".into());
        }
        ...
        let kernel = module.function(c"native_mul_chain")?;
