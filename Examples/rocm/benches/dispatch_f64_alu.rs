//! Compare a scalar f64 PCU Add kernel with independent native HIP.
...
#[pcu(invocations = N)]
fn add_f64<const N: usize>(left: &[f64], right: &[f64], output: &mut [f64]) {
...
fn bench(criterion: &mut Criterion) {
    run(criterion).expect("paired f64 benchmark failed");
}
...
fn run_case<const N: usize>(
    ...
    let left = (0..N)
        .map(|index| f64::from(u16::try_from(index % 17).expect("small residue")) / 8.0 - 1.0)
        .collect::<Vec<_>>();
    let right = (0..N)
        .map(|index| f64::from(u16::try_from(index % 13).expect("small residue")) / 16.0)
        .collect::<Vec<_>>();
    let expected = left
        .iter()
        .zip(&right)
    ...
    let source = format!(
        "#include <hip/hip_runtime.h>\nextern \"C\" __global__ void native_f64_add_mul(const double* left, const double* right, double* output) {{\n    const unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;\n    if (id < {N}u) output[id] = left[id] + right[id];\n}}\n"
    );
    let image = compile_hip_source(&source, architecture)?;
    let module = runtime.load_module(&image)?;
    let native_function = module.function(c"native_f64_add_mul")?;
    ...
    if pcu_bytes != expected_bytes || native_bytes != expected_bytes {
        return Err("f64 PCU/native Add output mismatch".into());
    }
    println!("f64 Add verified for {N} values, including positive and negative finite values");

    {
        let mut group = criterion.benchmark_group("f64-add");
                ...
                bencher.iter_custom(|iterations| {
                    ...
                    for iteration in 0..iterations {
                        let (pcu, native) = if iteration.is_multiple_of(2) {
                            ...
                            .expect("PCU f64 launch failed");
                            let native =
                                run_direct(&native_function, &stream, &native_arguments, grid)
                                    .expect("native f64 launch failed");
                            (pcu, native)
                        } else {
                            let native =
                                run_direct(&native_function, &stream, &native_arguments, grid)
