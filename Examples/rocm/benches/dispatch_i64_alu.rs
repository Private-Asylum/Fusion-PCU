//! Compare a chained wrapping i64 PCU kernel with independent native HIP.
...
#[pcu(invocations = N)]
fn wrapping_add_mul_i64<const N: usize>(left: &[i64], right: &[i64], output: &mut [i64]) {
...
fn bench(criterion: &mut Criterion) {
    run(criterion).expect("paired i64 benchmark failed");
}
...
fn run(criterion: &mut Criterion) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "i64 wrapping Add/Mul device: {} ({architecture})",
...
fn run_case<const N: usize>(
    ...
    let left = (0..N)
        .map(|index| i64::try_from(index).map(|value| i64::MAX.wrapping_sub(value)))
        .collect::<Result<Vec<_>, _>>()?;
    let right = (0..N)
        .map(|index| i64::try_from(index).map(|value| value.wrapping_mul(17).wrapping_add(3)))
        .collect::<Result<Vec<_>, _>>()?;
    let expected = left
        .iter()
        .zip(&right)
    ...
    let bindings = wrapping_add_mul_i64_bindings();
    let builder = wrapping_add_mul_i64::<N>(&bindings)?;
    let kernel = builder.ir();
    let invocations = u32::try_from(N)?;
    let prepared = support::cold_once("PCU i64 cold prepare", || {
    ...
    let source = format!(
        "#include <hip/hip_runtime.h>\nextern \"C\" __global__ void native_i64_add_mul(const unsigned long long* left, const unsigned long long* right, unsigned long long* output) {{\n    const unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;\n    if (id < {N}u) output[id] = (left[id] + right[id]) * right[id];\n}}\n"
    );
    let image = compile_hip_source(&source, architecture)?;
    let module = runtime.load_module(&image)?;
    let native_function = module.function(c"native_i64_add_mul")?;
    ...
    if pcu_bytes != expected_bytes || native_bytes != expected_bytes {
        return Err("i64 PCU/native wrapping Add/Mul output mismatch".into());
    }
    println!("i64 wrapping Add/Mul verified for {N} values, including overflow");

    {
        let mut group = criterion.benchmark_group("i64-wrapping-add-mul-chain");
                ...
                bencher.iter_custom(|iterations| {
                    ...
                    for iteration in 0..iterations {
                        let (pcu, native) = if iteration.is_multiple_of(2) {
                            ...
                            .expect("PCU i64 launch failed");
                            let native =
                                run_direct(&native_function, &stream, &native_arguments, grid)
                                    .expect("native i64 launch failed");
                            (pcu, native)
                        } else {
                            let native =
                                run_direct(&native_function, &stream, &native_arguments, grid)
