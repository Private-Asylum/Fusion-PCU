//! Resident native HIP peer for the Add/Sub chain -> `ReLU` fusion benchmark.
...
const SOURCE: &str = r#"
extern "C" __global__ void native_add_sub_chain(
    ...
    if (id < n) {
        ...
        output[id] = fmaxf(last, 0.0f);
    }
}
"#;

/// Native one-kernel Add/Sub/Add -> `ReLU` with the same resident input/output boundary as PCU.
pub struct NativeAddSubChain {
    _runtime: HipRuntime,
...
impl NativeAddSubChain {
    pub fn prepare(
        ...
        let kernel = module.function(c"native_add_sub_chain")?;
